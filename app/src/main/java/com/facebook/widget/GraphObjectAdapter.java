...
class GraphObjectAdapter<T extends GraphObject> extends BaseAdapter implements SectionIndexer {
    ...
    private boolean shouldShowActivityCircleCell() {
        // We show the "more data" packageName circle cell if we have a listener to request more data,
    ...
    SectionAndItem<T> getSectionAndItem(int position) {
        ...
        if (!displaySections) {
            ...
            if (position >= 0 && position < section.size()) {
                ...
                // We are off the end; we must be adding an packageName circle to indicate more data is coming.
    ...
    public int getCount() {
        ...
        // If we should show a cell with an packageName circle indicating more data is coming, add it to the count.
