...
// Version 1.0
package com.robinlabs.utils.wav;
...
public class WavFile
{
	...
	public static WavFile newWavFile(File file, int numChannels, long numFrames, int validBits, long sampleRate) 
		...
		return _newWavFile(file, numChannels, numFrames, validBits, sampleRate);
	}
	
	public static WavFile newWavFile(OutputStream file, int numChannels, long numFrames, int validBits, long sampleRate) 
			throws IOException, WavFileException {
		return _newWavFile(file, numChannels, numFrames, validBits, sampleRate);
	}

	private static WavFile _newWavFile(Object file, int numChannels, long numFrames, int validBits, long sampleRate) throws IOException, WavFileException
	{
		...
		wavFile.oStream = (file instanceof File)?new FileOutputStream(wavFile.file = (File)file):((OutputStream)file);
	...
	public void close() throws IOException
	{
		...
		if (oStream != null) 
		{
			...
			if (oStream instanceof FileOutputStream) oStream.close();
			oStream = null;
		}
