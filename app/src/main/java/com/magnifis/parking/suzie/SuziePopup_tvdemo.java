...
*/
public class SuziePopup_tvdemo implements OnTouchListener {
	...
	private static WeakReference<SuziePopup_tvdemo> selfWr=null;

	public static SuziePopup_tvdemo get() {
...
//	private WindowManager.LayoutParams _paramsBg = null;
	...
	public SuziePopup_tvdemo(SuzieService service) {
		this.service=service;
		selfWr = new WeakReference<SuziePopup_tvdemo>(this);
		_theApp = service.getApplication();
		
		buttonPaddingInner=App.self.toPxSize(5);
...
//					_layoutBubles.invalidate();
		...
		_layoutBubles.post(
				new Runnable() {
					...
					public void run() {
                        ...
                        LinearLayout bg = (LinearLayout) _layoutBubles.findViewById(R.id.bg);
						if (_paramsBtn.x > 0)
                            ((LinearLayout.LayoutParams)bg.getLayoutParams()).setMargins(_layoutBtnBox.getWidth(), 0, 0, 0);
						else
                            ((LinearLayout.LayoutParams)bg.getLayoutParams()).setMargins(0, 0, _layoutBtnBox.getWidth(), 0);
    ...
    public void bubleQueryCorrect(final String s) {
		...
		_layoutBubles.post(new Runnable() {
			...
			public void run() {
		    	...
		    	String str = s; 
		    	if (str.toLowerCase().startsWith("moviebot "))
		    		str = s.replace("moviebot ", ""); 
		    	
		    	tv.setText(Utils.firstUpper(str, false));
			}
		});
	}
	...
	static public void enableBubles() {
		bublesEnabled = true;
		final SuziePopup_tvdemo sp = SuziePopup_tvdemo.get();
...
/*
	...
	private void createButton() {
		...
		_layoutBtnBox.setPadding(buttonPaddingInner, buttonPaddingInner, buttonPaddingInner, buttonPaddingInner);
...
//		if (_layoutBg!=null) return;
//		
//		_paramsBg = new WindowManager.LayoutParams(
//				LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT,
//				
//				WindowManager.LayoutParams.TYPE_SYSTEM_ALERT,
//				
...
//				PixelFormat.TRANSLUCENT);
//
//		_paramsBg.width = LayoutParams.MATCH_PARENT;
//		_paramsBg.height = LayoutParams.WRAP_CONTENT;
//		_paramsBg.format = PixelFormat.TRANSLUCENT;
//		_paramsBg.gravity = Gravity.TOP | Gravity.RIGHT;
//		_paramsBg.y = actionBarHeight;
//		_paramsBg.x = 0;
//		_paramsBg.alpha = 1;
//
//		LayoutInflater inflater = (LayoutInflater)service.getSystemService( Context.LAYOUT_INFLATER_SERVICE ); 
//		_layoutBg = (LinearLayout) inflater.inflate(R.layout.suzie_buble_bg, null);
//		_windowManager.addView(_layoutBg, _paramsBg);
//		_layoutBg.findViewById(R.id.buble_bg).setVisibility(View.VISIBLE);
//	}
	...
	private static void _hideBubles(SuziePopup_tvdemo sp, boolean fast, boolean hideWeather) {
...
//		_paramsBg.x = 0;
//		_paramsBg.y = actionBarHeight;
//		_windowManager.updateViewLayout(_layoutBg, _paramsBg);
//		_layoutBg.setVisibility(View.VISIBLE);
//	}
	...
	public static void startAutoHideTimer() {
		final SuziePopup_tvdemo sp = get();
		if (sp == null)
			return;
