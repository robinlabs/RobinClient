...
public abstract class PickerFragment<T extends GraphObject> extends Fragment {
    ...
    void layoutActivityCircle() {
        // If we've got no data, make the packageName circle full-opacity. Otherwise we'll dim it to avoid
        //  cluttering the UI.
    ...
    void hideActivityCircle() {
        if (activityCircle != null) {
            // We use an animation to dim the packageName circle; need to clear this or it will remain visible.
    ...
    void updateAdapter(SimpleGraphObjectCursor<T> data) {
        if (adapter != null) {
            ...
            // We use the second row, to give packageName above and below it and avoid
            // cases where the first row is only barely visible, thus providing little packageName.
